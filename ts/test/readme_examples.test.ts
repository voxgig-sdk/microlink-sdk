...
//     (\`new MicrolinkSDK(...)\` / \`MicrolinkSDK.test(...)\` become
//     \`MicrolinkSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_website_data":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MicrolinkSDK(...)\` and any
// \`MicrolinkSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MicrolinkSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MicrolinkSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
