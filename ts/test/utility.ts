...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MICROLINK_TEST_LIVE ||
    'TRUE' === process.env.MICROLINK_TEST_OVERRIDE
  ) {
  ...
  m.MICROLINK_TEST_EXPLAIN = process.env.MICROLINK_TEST_EXPLAIN || m.MICROLINK_TEST_EXPLAIN
