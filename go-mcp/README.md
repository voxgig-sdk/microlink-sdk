...
MCP server exposing the Microlink SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `microlink_list` | `entity`, optional `query` map | First page of records as JSON |
| `microlink_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `microlink_list` and
`microlink_load` tools become available in new sessions.
