...
class TestGetWebsiteDataEntity:

    def test_should_create_instance(self):
        testsdk = MicrolinkSDK.test(None, None)
        ent = testsdk.GetWebsiteData(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _get_website_data_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "get_website_data." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MICROLINK_TEST_GET_WEBSITE_DATA_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        get_website_data_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.get_website_data")))
        get_website_data_ref01_data = None
        if len(get_website_data_ref01_data_raw) > 0:
...
def _get_website_data_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/get_website_data/GetWebsiteDataTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("MICROLINK_TEST_GET_WEBSITE_DATA_ENTID"))
    ...
    if env.get("MICROLINK_TEST_LIVE") == "TRUE":
        ...
        client = MicrolinkSDK(helpers.to_map(merged_opts))

    _live = env.get("MICROLINK_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("MICROLINK_TEST_EXPLAIN") == "TRUE",
        "live": _live,
