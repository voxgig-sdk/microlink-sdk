...
local MicrolinkSDK = {}
MicrolinkSDK.__index = MicrolinkSDK
...
MicrolinkSDK._make_feature = _make_feature


function MicrolinkSDK.new(options)
  local self = setmetatable({}, MicrolinkSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function MicrolinkSDK:options_map()
...
function MicrolinkSDK:get_utility()
  return Utility.copy(self._utility)
end


function MicrolinkSDK:get_root_ctx()
  return self._rootctx
end


function MicrolinkSDK:prepare(fetchargs)
...
function MicrolinkSDK:direct(fetchargs)
...
-- by passing it.
function MicrolinkSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function MicrolinkSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "MicrolinkSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function MicrolinkSDK:GetWebsiteData(data)
  ...
  if data == nil then
    if self._get_website_data == nil then
      self._get_website_data = EntityMod.new(self, nil)
    end
...
function MicrolinkSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = MicrolinkSDK.new(sdkopts)
  sdk.mode = "test"
