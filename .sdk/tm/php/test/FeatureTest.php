...
class FtCtrl extends MicrolinkControl
{
...
class FtClient
{
    ...
    private MicrolinkUtility $utility;

    public function __construct(MicrolinkUtility $utility, array $options)
    {
    ...
    public function get_utility(): MicrolinkUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MicrolinkContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new MicrolinkUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (MicrolinkContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MicrolinkConfig::make_feature($name);
    ...
    private static function buildUrl(MicrolinkSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MicrolinkError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MicrolinkResponse($fetched);
        }
    ...
    private function fail(MicrolinkContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MicrolinkContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
