// Command Microlink-cli is a CLI + REPL for the Microlink SDK,
...
const prompt = "microlink"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_website_data"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MICROLINK_APIKEY (api key), MICROLINK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MICROLINK_APIKEY carries the API key and
	// MICROLINK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MICROLINK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MICROLINK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMicrolinkSDK(opts)
