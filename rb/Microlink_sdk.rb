...
require_relative 'Microlink_types'
...
class MicrolinkSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MicrolinkHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MicrolinkHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MicrolinkFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MicrolinkUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MicrolinkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MicrolinkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MicrolinkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MicrolinkHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MicrolinkError.new(
        "graphql_error", "MicrolinkSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
