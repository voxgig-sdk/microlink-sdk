...
module MicrolinkUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MicrolinkResponse.new({})
    ...
    if fetch_err
      ...
      response = MicrolinkResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MicrolinkResponse.new(fetched)
    else
