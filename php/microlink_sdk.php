...
class MicrolinkSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = MicrolinkHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MicrolinkHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ($utility->feature_add)($this->_rootctx, MicrolinkFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MicrolinkUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MicrolinkHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MicrolinkHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MicrolinkHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MicrolinkError($op . "_allow",
                "MicrolinkSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MicrolinkHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MicrolinkError("graphql_error",
                "MicrolinkSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetWebsiteData($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_website_data === null) {
                $this->_get_website_data = new GetWebsiteDataEntity($this, null);
            }
            return $this->_get_website_data;
        }
        return new GetWebsiteDataEntity($this, $data);
    }
