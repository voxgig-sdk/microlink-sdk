...
class GetWebsiteDataEntity
  ...
  def make
    opts = @_entopts.dup
    GetWebsiteDataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MicrolinkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetWebsiteData, Hash] the current GetWebsiteData data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MicrolinkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetWebsiteData fields)
  def match_get
  ...
  #
  # @param reqmatch [GetWebsiteDataLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetWebsiteData.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetWebsiteData, Hash] the loaded GetWebsiteData; raises MicrolinkError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MicrolinkHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
