...
[MCP](https://modelcontextprotocol.io) server exposing the Microlink SDK as
two agent tools — `microlink_list` and `microlink_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/microlink-mcp)
make build

# 2. Provide credentials via the environment
export MICROLINK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user microlink \
  -- /absolute/path/to/microlink-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./microlink-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// microlink_list: first page of records
{ "entity": "get_website_data" }
{ "entity": "get_website_data", "query": { } }

// microlink_load: one record by id
{ "entity": "get_website_data", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/microlink-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user microlink \
     -- "$PWD"/dist/*/microlink-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `microlink_list` and `microlink_load` tools now appear
   in new sessions. Ask the agent to *"list get_website_data using microlink"*
   and it calls `microlink_list` with `{"entity":"get_website_data"}`.
...
```sh
export MICROLINK_APIKEY=sk_live_xxx            # API key
export MICROLINK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `microlink_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `microlink_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MICROLINK_APIKEY` | API key sent with every request. |
| `MICROLINK_BASE` | Optional override of the API base URL. |
...
```sh
./microlink-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"microlink_load","arguments":{"entity":"get_website_data","query":{"id":1}}}}'
```
