# Microlink SDK test runner

from __future__ import annotations
import os
import json

from microlink_sdk.utility.voxgig_struct import voxgig_struct as vs


class MicrolinkTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MicrolinkTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MicrolinkTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MicrolinkTestRunner.getenv("MICROLINK_TEST_LIVE")
        override = MicrolinkTestRunner.getenv("MICROLINK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MicrolinkTestRunner.getenv(key)
        ...
        explain = MicrolinkTestRunner.getenv("MICROLINK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MICROLINK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MicrolinkTestRunner._test_control is not None:
            return MicrolinkTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MicrolinkTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MicrolinkTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MicrolinkTestRunner.load_test_control()
...
def load_env_local():
    MicrolinkTestRunner.load_env_local()


def env_override(m):
    return MicrolinkTestRunner.env_override(m)


def entity_data(v):
    return MicrolinkTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MicrolinkTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MicrolinkTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MicrolinkTestRunner.load_test_control()


def live_delay_ms():
    return MicrolinkTestRunner.live_delay_ms()
