...
boru-driven command-line client **and** interactive REPL for the Microlink
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/microlink-cli)
make build
...
./microlink-cli load 1 get_website_data            # {id:1} shorthand
./microlink-cli load '{id:1}' get_website_data       # explicit match map

# 5. Override the API base URL for a single call
MICROLINK_BASE=https://api.example.com ./microlink-cli load 1 get_website_data

# 6. No arguments -> interactive REPL
./microlink-cli
microlink> load 1 get_website_data
microlink> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/microlink-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/microlink-cli load 1 get_website_data
   ```
...
```sh
./microlink-cli load 1 get_website_data          # scalar shorthand for {id:1}
./microlink-cli load '{id:1}' get_website_data     # explicit match map
```
...
./microlink-cli load 1 get_website_data
```
...
Run with no arguments to open a REPL (prompt `microlink>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_website_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/microlink-cli`. |
...
(`./microlink-cli <expr>`) and interactively (the REPL), and expressions compose the
