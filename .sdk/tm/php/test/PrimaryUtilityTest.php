...
// Microlink SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        MicrolinkSDK $client,
        MicrolinkUtility $utility
    ): MicrolinkContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new MicrolinkContext($ctxmap, null);
    ...
    private static function fixctx(MicrolinkContext $ctx, MicrolinkSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?MicrolinkError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MicrolinkError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new MicrolinkResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new MicrolinkBaseFeature();
    ...
    {
        $hook_client = MicrolinkSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends MicrolinkBaseFeature {
            public $hook_fn = null;
            public function TestHook(MicrolinkContext $ctx): void
            {
    ...
    {
        $init_client = MicrolinkSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends MicrolinkBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new MicrolinkSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new MicrolinkSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = MicrolinkSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = MicrolinkSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof MicrolinkError) ? $result->err->msg : (string)$result->err;
